...
import { Database, Upload, RefreshCw, Search, TrendingUp, Cpu } from 'lucide-react';
...
export default function KnowledgePage() {
  ...
  const handleSearch = async () => {
    ...
    setSearching(true);
    try {
      const response = await fetch(`/api/knowledge/search?q=${encodeURIComponent(searchQuery)}&top_k=5`);
      const data = await response.json();
  ...
  const handleRetrain = async () => {
    ...
    try {
      ...
      if (data.success) {
        await loadStats();
        alert(`✅ Successfully retrained ${data.documents_retrained} documents!`);
      } else {
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-black mb-2">Knowledge Base Management</h1>
              <p className="text-black">
                Powered by OPEA AI on <span className="inline-flex items-center gap-2 font-semibold text-blue-600">
                  <Cpu size={20} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
            ...
            <div className="text-3xl font-bold text-black">{stats?.total_documents || 0}</div>
            <div className="text-sm text-black">Total Documents</div>
          </div>
          
          <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
            <TrendingUp size={32} className="text-green-600 mb-2" />
            <div className="text-3xl font-bold text-black">{stats?.training_runs || 0}</div>
            <div className="text-sm text-black">Training Runs</div>
          </div>
          
          <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
            <Upload size={32} className="text-purple-600 mb-2" />
            <div className="text-3xl font-bold text-black">{stats?.recent_runs?.length || 0}</div>
            ...
            {activeTab === 'manage' && (
              <div className="space-y-6">
                <div className="bg-white rounded-xl border border-gray-200 p-6">
                  <h3 className="text-xl font-bold text-black mb-4">Retrain Knowledge Base</h3>
                  <p className="text-black mb-4">
                    Retrain the entire knowledge base with new embeddings. This ensures all data is indexed with the latest model.
                ...
                {stats && stats.recent_runs && stats.recent_runs.length > 0 && (
                  <div className="bg-white rounded-xl border border-gray-200 p-6">
                    <h3 className="text-xl font-bold text-black mb-4">Recent Training Runs</h3>
                    <div className="space-y-2">
                      {stats.recent_runs.map((run: any, idx: number) => (
                        <div key={idx} className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                          <div className="flex justify-between items-center">
                            <div>
                              <div className="text-sm font-semibold text-black">{run.source || run.type}</div>
                              <div className="text-xs text-black">
                                {run.documents_added || run.documents_processed} documents
                              </div>
                            </div>
                            <div className="text-xs text-black">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl p-6 text-white shadow-lg">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="text-2xl font-bold mb-2">Intel Xeon Performance</h3>
              <p className="text-white/90 mb-4">
                This system leverages Intel Xeon processors with AI acceleration for superior knowledge processing performance:
              </p>
              <ul className="space-y-2 text-white/90">
                <li className="flex items-start gap-2">
                  <span className="text-yellow-300 font-bold">⚡</span>
                  <span>Optimized embedding generation with Intel libraries</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-yellow-300 font-bold">⚡</span>
                  <span>Fast vector similarity search powered by Intel optimizations</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-yellow-300 font-bold">⚡</span>
                  <span>Efficient document processing with Intel Xeon compute power</span>
                </li>
              </ul>
            </div>
          </div>
