...
class CodeTranslationAPIRouter(APIRouter):
    ...
    def handle_code_translation(self, 
                                language_from: str,
                                language_to: str,
                                source_code: str):
        prompt = self.prompt_template.format(language_from=language_from,
    ...
    async def handle_code_translation_stream(self, 
                                language_from: str,
                                language_to: str,
                                source_code: str):
        prompt = self.prompt_template.format(language_from=language_from,
...
router = CodeTranslationAPIRouter(entrypoint=TGI_ENDPOINT,
                                    prompt_template=codetrans_prompt_template)
...
async def code_translation(request: Request):
    ...
    try:
        return router.handle_code_translation(language_from=language_from,
                                            language_to=language_to,
                                            source_code=source_code)
    except Exception as e:
        print(f"[codetrans - nonstream] Error occurred: {e}")
...
async def code_translation_stream(request: Request):
    ...
    source_code = params["source_code"]
    try:
        return await router.handle_code_translation_stream(language_from=language_from,
                                                language_to=language_to,
                                                source_code=source_code)
    except Exception as e:
