...
async def update_knowledge_base(knowledge: KnowledgeBaseCreateIn):
    try:
        ...
        if active_pl.indexer.comp_subtype == "kbadmin_indexer" and kb.comp_subtype != "kbadmin_kb":
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="The kbadmin pipeline must correspond to the kbadmin type kb.")
        if active_pl.indexer.comp_subtype != "kbadmin_indexer" and kb.comp_subtype == "kbadmin_kb":
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Not kbadmin pipeline cannot active kbadmin type kb.")
...
async def add_file_to_knowledge_base(knowledge_name, file_path: DataIn):
    try:
        ...
        if kb.comp_subtype == "kbadmin_kb" or active_pl.indexer.comp_subtype == "kbadmin_indexer":
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Please proceed to the kbadmin interface to perform the operation.")
...
async def save_knowledge_to_file():
    ...
    try:
        ...
        knowledgebases_data = []
        for kb in kb_base:
            kb_json = {"name": kb.name, "description": kb.description, "active": kb.active, "file_map": kb.file_map, "comp_type": kb.comp_type, "comp_subtype":kb.comp_subtype, "experience_active": kb.experience_active}
...
async def Synchronizing_vector_data(old_active_pl, new_active_pl, pl_change):
    try:
       ...
       # Determine whether it is kbadmin type
        if old_active_pl:
            if old_active_pl.retriever.comp_subtype == "kbadmin_retriever" and new_active_pl.retriever.comp_subtype == "kbadmin_retriever":
                if active_kb:
