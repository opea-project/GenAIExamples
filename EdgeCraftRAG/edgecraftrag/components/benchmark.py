...
from edgecraftrag.base import BaseComponent, InferenceType, CompType, ModelType
    ...
    def ser_model(self):
        if self.enabled:
            if self.is_vllm:
                ...
                "last_benchmark_data": self.benchmark_data_list[self.last_idx] if self.last_idx in self.benchmark_data_list else None,
...
def get_vllm_metrics():
    ...
    if "vllm:time_to_first_token_seconds_sum" in metrics and "vllm:time_to_first_token_seconds_count" in metrics:
        metrics["average_time_to_first_token_seconds"] = metrics["vllm:time_to_first_token_seconds_sum"] / metrics["vllm:time_to_first_token_seconds_count"] if metrics["vllm:time_to_first_token_seconds_count"] > 0 else None
    if "vllm:time_per_output_token_seconds_sum" in metrics and "vllm:time_per_output_token_seconds_count" in metrics:
        metrics["average_time_per_output_token_seconds"] = metrics["vllm:time_per_output_token_seconds_sum"] / metrics["vllm:time_per_output_token_seconds_count"] if metrics["vllm:time_per_output_token_seconds_count"] > 0 else None
    if "vllm:e2e_request_latency_seconds_sum" in metrics and "vllm:e2e_request_latency_seconds_count" in metrics:
        metrics["average_e2e_request_latency_seconds"] = metrics["vllm:e2e_request_latency_seconds_sum"] / metrics["vllm:e2e_request_latency_seconds_count"] if metrics["vllm:e2e_request_latency_seconds_count"] > 0 else None
