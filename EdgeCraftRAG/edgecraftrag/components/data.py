...
def convert_file_to_documents(file_path) -> List[Document]:
    ...
    supported_exts = [".pdf", ".txt", ".doc", ".docx",
                        ...
                        ".html", ".rst"]
    if file_path.is_dir():
        docs = SimpleDirectoryReader(input_dir=file_path,
                        recursive=True,
                        required_exts=supported_exts).load_data()
    elif file_path.is_file():
        docs = SimpleDirectoryReader(input_files=[file_path],
                        required_exts=supported_exts).load_data()
    else:
        docs = []
