...
def postproc_query(response_text, state):
    ...
    try:
        ...
        log_status("🧲", f"{format_terminal_str('Keywords for retrieval:', color='magenta')} {format_terminal_str(keywords_str, italic=True)}")
        log_status("🔮", f"{format_terminal_str('Query for reranking:', color='magenta')} {format_terminal_str(query_str, italic=True)}")
