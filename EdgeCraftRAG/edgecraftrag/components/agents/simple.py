...
from edgecraftrag.base import CallbackType, CompType
from edgecraftrag.components.agent import Agent, stream_writer
from edgecraftrag.components.agents.utils import format_terminal_str, ROLE
...
class SimpleRAGAgent(Agent):
    ...
    async def retrieve(self, state: QnaState) -> dict:
        ...
        print("🔍", f"Retrieved {format_terminal_str(str(len(retrieved)), color='magenta', bold=True)} documents, Reranked to top {format_terminal_str(str(len(reranked)), color='magenta', bold=True)}.")
        await stream_writer(f"\n\n🔍 **Retrieved {str(len(retrieved))} documents, Reranked to top {str(len(reranked))}**\n\n")

        new_retrieval = Retrieval(
            step=state.num_retrievals,
            query=state.query,
            retrieved=retrieved,
            reranked=reranked
        )
    ...
    async def generate_query(self, state: QnaState) -> dict:
        ...
        messages = [
            ...
            {
                "role": ROLE.SYSTEM,
                "content": PROMPT_TEMPLATE.GENERATE_QUERY
            }
        ]
    ...
    async def check_retrieved(self, state: QnaState) -> str:
        ...
        contexts = PROMPT_TEMPLATE.CONTEXTS.format(
            contexts='\n'.join([
                PROMPT_TEMPLATE.CONTEXT.format(context=doc.text)
        ...
        messages = [
            ...
            {
                "role": ROLE.SYSTEM,
                "content": PROMPT_TEMPLATE.CONTINUE
            }
        ]
        self._messages.extend(messages)
        self.conversation_history.extend(messages)
        if state.num_retrievals >= self.max_retrievals:
            print("⚠️", format_terminal_str(f"Reached maximum retrievals: {self.max_retrievals}, stopping search\n", color="yellow", bold=True))
    ...
    async def run(self, **kwargs) -> Any:
        if "cbtype" in kwargs:
            if kwargs["cbtype"] == CallbackType.RUNAGENT:
                ...
                print("🤿", f"{format_terminal_str('Starting DeepSearch:', color='cyan', bold=True)} {format_terminal_str(request.messages, bold=False, italic=True)}\n")
                # Initialize state
                state = QnaState(
