    ...
    <div v-if="failedFiles.length" class="failed-container">
      <div class="failed-files-header">
        <span class="failed-title">
          ...
          <span class="tag-wrap">
            {{ $t("common.count") }}: {{ failedFiles.length }}</span
          >
        </span>
          ...
          <div class="left-wrap">
            <div class="header-wrap">
              ...
              <div class="file-info">
                ...
                <div class="file-path">
                  <BranchesOutlined class="path-icon" />{{ file.path }}
                </div>
          ...
          <div v-for="file in knowledgeData" :key="file.name" class="file-item">
            ...
            <div class="right-wrap">
              <a-tooltip placement="top" :title="$t('common.delete')">
                <DeleteFilled
                  class="delete-icon"
                  @click="handleFileDelete(file.path as string)"
                />
              </a-tooltip>
            </div>
          </div>
...
import { Modal, UploadFile } from "ant-design-vue";
...
const handleBeforeUpload = (file: UploadFile) => {
  const isFileSize = (file.size || 0) / 1024 / 1024 < 200;
  if (!isFileSize) {
...
const handleFailedFile = (file: File, path: string, error: string) => {
  ...
  const existingIndex = failedFiles.value.findIndex(
    ...
    await Promise.all(
      batch.map(async (options: any) => {
        ...
        try {
          ...
          await requestKnowledgeBaseRelation(kbInfo?.name!, {
            local_path: res,
          });
...
const ignoreSingleFile = async (failedFile: FailedFile) => {
  ...
  try {
    const index = failedFiles.value.findIndex(
      (item) => item.name === failedFile.name,
    );
    if (index !== -1) {
...
const replaceAllFailedFiles = async () => {
  ...
  try {
    ...
    const filesToRetry = failedFiles.value.filter((_file, index) => {
...
const handleSizeChange = (_page: number, size: number) => {
