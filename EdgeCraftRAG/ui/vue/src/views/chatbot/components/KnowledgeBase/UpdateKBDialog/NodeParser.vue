<template>
  <a-form
    ...
    class="form-wrap"
  >
    <a-form-item :label="$t('pipeline.config.nodeParserType')" name="parser_type">
      <a-select
        showSearch
        v-model:value="form.parser_type"
        :placeholder="$t('pipeline.valid.nodeParserType')"
      >
        <a-select-option v-for="item in nodeParserList" :key="item.value" :value="item.value">{{
          item.name
        }}</a-select-option>
      </a-select>
    ...
    <template v-if="!isHierarchical && !isSentencewindow">
      <a-form-item :label="$t('pipeline.config.chunkSize')" name="chunk_size" class="slider-wrap">
        <a-slider
    ...
    <template v-if="isSentencewindow">
      <a-form-item :label="$t('pipeline.config.windowSize')" name="window_size" class="slider-wrap">
        <a-slider v-model:value="form.window_size" :min="1" :max="10" :marks="sliderMarks.window" />
        <a-form-item noStyle>
          <a-input-number
...
<script lang="ts" setup name="NodeParser">
  ...
  const formatFormParam = () => {
    const { parser_type, chunk_size, chunk_overlap, chunk_sizes, window_size } = form;

    const chunkSizes = isHierarchical.value ? chunk_sizes : undefined;
    const windowSize = isSentencewindow.value ? window_size : undefined;

    const chunkSize = !isHierarchical.value && !isSentencewindow.value ? chunk_size : undefined;
