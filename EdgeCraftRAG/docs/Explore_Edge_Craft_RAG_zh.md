...
然后按照 UI 中的流水线创建向导进行配置。请注意，在 `Indexer Type`（索引器类型）中可以选择 MilvusVector 作为索引器（请确保在选择 MilvusVector 之前已启用 Milvus，可参考 [启用 Milvus](Advanced_Setup_zh.md#使用-docker-compose-在-intel-gpu-上部署服务)）。  
...
在 `Node Parser` 页面，选择 `kbadmin` 作为节点解析器。
...
请在 `Type` 中选择 `kbadmin`，并从 kbadmin UI 页面中已创建的知识库列表中选择 kb 名称。
