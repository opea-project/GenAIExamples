...
const Conversation = ({ title }: ConversationProps) => {
  ...
  const [fileUploadOpened, { open: openFileUpload, close: closeFileUpload }] =
    useDisclosure(false);

  const { conversations, onGoingResult, selectedConversationId } =
    useAppSelector(conversationSelector);
  const dispatch = useAppDispatch();
  const selectedConversation = conversations.find(
    (x) => x.conversationId === selectedConversationId,
  );
            ...
            {!selectedConversation && (
              <>
                ...
                <div className="infoMessage">
                  You can also upload your Document by clicking on Document icon
                  on top right corner
                </div>
              </>
            )}
