...
function build_docker_images() {
    ...
    docker ghcr.io/huggingface/text-generation-inference:2.4.1-rocm
...
function validate_services() {
    ...
    if [ "$HTTP_STATUS" -eq 200 ]; then
        ...
        local CONTENT=$(curl -s -X POST -d "$INPUT_DATA" -H 'Content-Type: application/json' "$URL" | tee ${LOG_PATH}/${SERV>
