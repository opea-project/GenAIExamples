...
After running docker compose, check if all the containers launched via docker compose has started:

```
docker ps -a
```

For the default deployment, the following 10 containers should be started:
...
The ChatQnA containers should be stopped and removed after execution finishes.
