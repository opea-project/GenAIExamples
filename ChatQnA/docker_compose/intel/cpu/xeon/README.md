...
1. Online
   ...
   docker run -p 8008:80 -v ./data:/data --name tgi-service -e HF_ENDPOINT=$HF_ENDPOINT -e http_proxy=$http_proxy -e https_proxy=$https_proxy --shm-size 1g ghcr.io/huggingface/text-generation-inference:latest-intel-cpu --model-id $model_name
   ```
...
2. Offline
   ...
   - Run the following command to start TGI service.
     ...
     docker run -p 8008:80 -v $model_path:/data --name tgi_service --shm-size 1g ghcr.io/huggingface/text-generation-inference:latest-intel-cpu --model-id /data
     ```
