...
1. Online
   ...
   docker run -p 8008:80 -v ./data:/data --name vllm-service -e HF_ENDPOINT=$HF_ENDPOINT -e http_proxy=$http_proxy -e https_proxy=$https_proxy --shm-size 1g opea/vllm:latest --model $model_name
...
2. Offline
   ...
   - Run the following command to start the LLM service.
     ...
     # Start vLLM LLM Service
     docker run -p 8008:80 -v $model_path:/data --name vllm-service --shm-size 1g opea/vllm:latest --model /data
     # Start TGI LLM Service
