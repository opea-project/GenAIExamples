...
Please follow this link [huggingface token](https://huggingface.co/docs/hub/security-tokens) to get the access token and export `HUGGING_FACE_HUB_TOKEN` environment with the token.
...
Update the `HUGGING_FACE_HUB_TOKEN` environment variable with your huggingface token in the `docker-compose.yml`
