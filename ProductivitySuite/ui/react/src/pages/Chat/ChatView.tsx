...
import { abortStream, conversationSelector, doCodeGen, doConversation, doSummaryFaq, getConversationHistory, newConversation, setSelectedConversationId } from '@redux/Conversation/ConversationSlice';
...
const ChatView = () => {
    ...
    const { selectedConversationHistory, type, sourceLinks, sourceFiles, temperature, 
        token, model, systemPrompt, selectedConversationId, onGoingResult, isPending } = useAppSelector(conversationSelector);
    ...
    useEffect(() => {
        ...
        if (conversation_id && conversation_id !== 'new') {
            ...
            dispatch(getConversationHistory({ user: name, conversationId: conversation_id }))
            scrollToTop();
            return;
    ...
    const displayMessage = () => {
        ...
        selectedConversationHistory.map((message, messageIndex) => {
            ...
            if (message.role !== MessageRole.System) {
                ...
                messagesDisplay.push(
                    <Box className={styles.messageContent} key={`${timestamp}_${messageIndex}`}>
                        {displayChatUser(message)}
                        {message.role === MessageRole.Assistant && <ChatAssistant message={message} />}
                    </Box>
                )
            }
        })
        ...
        if (onGoingResult) {
            ...
            messagesDisplay.push(
                <Box key={Math.random()} className={styles.messageContent}>
                    <ChatAssistant key={`ongoing_ai`} message={continueMessage} pending={true} />
                </Box>
            )
