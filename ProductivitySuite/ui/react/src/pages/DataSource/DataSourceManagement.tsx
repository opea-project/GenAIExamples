import { Box, Checkbox, FormControlLabel, List, ListItem, Typography, FormGroup } from "@mui/material";
import {  useTheme } from "@mui/material/styles";
import { useEffect, useState } from "react";
import styles from "./DataSourceManagement.module.scss";
import { useAppDispatch, useAppSelector } from "@redux/store";
import { conversationSelector, deleteInDataSource, getAllFilesInDataSource, deleteMultipleInDataSource } from "@redux/Conversation/ConversationSlice";
...
import { DeleteButton, SolidButton, TextButton} from "@root/shared/ActionButtons";
...
const DataSourceManagement = () => {
    ...
    const sortFiles = () => {
        if(activeSourceType === 'web'){
            let webFiles = filesInDataSource.filter((file)=>file.name.startsWith('http'));
            return webFiles
        } else {
            let otherFiles = filesInDataSource.filter((file)=>!file.name.startsWith('http'));
            return otherFiles
        };
    }
    ...
    const displayFiles = () => {
        ...
        return dataList.map((file: file) => {
            ...
            const controlCheckBox = (
                <Checkbox id={`check_${file.id}`} onChange={() => handleCheckboxChange(file.id)} checked={isChecked} />
            );

            return (
                <Box className={styles.dataItem} component={ListItem} key={file.id}>
                    {selectActive ? (
                        <FormControlLabel sx={{...theme.customStyles.gradientBlock}} className={styles.dataName} control={controlCheckBox} label={fileText} />
                    ) : (
                        <Typography variant="body1" component={'div'} sx={{...theme.customStyles.gradientBlock}} className={styles.dataName}>{fileText}</Typography>
    ...
    const handleSearch = (value: string) => {
        ...
        const searchResults = filteredList.filter((file: file) => file.name?.toLowerCase().includes(value.toLowerCase()));
    ...
    const displayInput = () => {
        let input = null;
        if (activeSourceType === 'documents') input = <FileInput dataManagement={true} />;
        if (activeSourceType === 'web') input = <DataWebInput />;
        if (activeSourceType === 'images') input = <FileInput dataManagement={true} imageInput={true} />;

        return (
            <Box className={styles.dataInputWrapper}>
                {input}
            </Box>
        )
    }
    ...
    return (
        <Box className={styles.dataView}>
                ...
                <FormGroup sx={{ marginRight: '1.5rem' }}>
                    <FormControlLabel
                        ...
                        control={<DropDown options={[
                            ...
                            ]} border={true} value={activeSourceType} handleChange={updateSource} />} />
                </FormGroup>
            </Box>
            ...
            <Box display={'flex'} flexDirection={'row'} justifyContent={'space-between'}>
                <Typography>You have {dataList.length} file{dataList.length !== 1 && 's'}</Typography>

                {dataList.length > 0 && (
                    <Box className={styles.actions}>
                        {selectActive
                            ? <TextButton onClick={() => handleSelectAll()}>Select All</TextButton>
                            : <TextButton onClick={() => setSelectActive(true)}>Select</TextButton>}

                        {selectActive && (
                            <>
                                <SolidButton onClick={() => cancelSelect()}>Cancel</SolidButton>
                                <DeleteButton onClick={() => deleteSelected()}>Delete Selected</DeleteButton>
                            </>
                        )}
                    </Box>
                )}
