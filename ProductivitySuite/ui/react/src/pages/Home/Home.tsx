...
import { conversationSelector, setType, newConversation } from '@redux/Conversation/ConversationSlice';
...
const Home = () => {
    ...
    const { type, types, token, model, temperature } = useAppSelector(conversationSelector)
    ...
    return (
        <div className={styles.homeView}>
            ...
            <HomeTitle className={styles.title} variant="h1">Hi, {name}. {config.tagline}</HomeTitle>

            <Grid2 className={styles.buttonRow} container spacing={2}>
                {types.map((interactionType, index) => (
                    <HomeButton key={index}
                        onClick={() => handleTypeChange(interactionType.key)}
                        aria-selected={type === interactionType.key}
                        startIcon={<interactionType.icon sx={{ fill: interactionType.color }} />}
                        variant="contained">{interactionType.name}</HomeButton>
                ))}
            </Grid2>
            ...
            <div className={styles.disclaimer} dangerouslySetInnerHTML={{ __html: disclaimer }}/>
