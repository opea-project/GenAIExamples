...
import { conversationSelector, newConversation } from "@redux/Conversation/ConversationSlice";
...
export const NavIcon: React.FC<NavIconProps> = ({ component: ListItemIcon }) => {
    const theme = useTheme()
    return <ListItemIcon sx={{fill: theme.customStyles.icon?.main}}/>;
};

const EmptySvg: React.FC = () => {
    return <svg className={styles.emptySvg} viewBox="0 0 24 24" width={'24px'} height={'24px'}></svg>;
};
...
export const LinkedMenuItem: React.FC<LinkedMenuItemProps> = ({ to, children, onClick, sx, open }) => {

    return (
        <MenuItem sx={sx}>
            <Link to={to} onClick={onClick} tabIndex={open ? 0 : -1} aria-hidden={!open}>
                {children}
            </Link>
        </MenuItem>
    );
...
const SideBar: React.FC<SideBarProps> = ({ asideOpen, setAsideOpen = ()=>{} , userDetails}) => {
    ...
    const handleLinkedMenuItemClick = (event: React.MouseEvent<HTMLLIElement>) => {
    ...
    const viewAll = (path: string) => {
        ...
        if(conversations.length > 0) {
            return (
                <LinkedMenuItem
                    open={asideOpen}
                    to={path} sx={{ marginBottom: '2rem' }} onClick={handleLinkedMenuItemClick}>
                    <NavIcon component={EmptySvg} />
                    <ListItemText className={styles.viewAll}>View All <KeyboardBackspace fontSize="small"/></ListItemText>
                </LinkedMenuItem>
            )
        } else {
            return (
    ...
    return (
        <aside className={`${styles.aside} ${asideOpen ? styles.open : ''}`} style={asideBackgroundColor}>
            <MenuList className={styles.asideContent}>
                ...
                { role === 'Admin' && (
                    <>
                        <LinkedMenuItem
                            open={asideOpen}
                            to="/data" onClick={handleLinkedMenuItemClick}>
...
const SideBarSpacer: React.FC<SideBarProps> = ({ asideOpen }) => {
    ...
    return (
        <div className={`${styles.asideSpacer} ${asideOpen ? styles.asideSpacerOpen : ''}`}></div>
    );
};
