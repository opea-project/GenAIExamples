...
import { Button, Typography, IconButton, Box, styled, Tooltip } from "@mui/material";
...
import { conversationSelector, saveConversationtoDatabase, setSelectedConversationHistory } from "@redux/Conversation/ConversationSlice";
...
const ChatAssistant: React.FC<ChatMessageProps> = ({ message, pending = false }) => {

    const dispatch = useAppDispatch();
    const { onGoingResult, selectedConversationHistory, selectedConversationId, type } = useAppSelector(conversationSelector)
    ...
    const saveResponse = () => {
        ...
        const convoClone: Message[] = selectedConversationHistory.map((messageItem) => {
            ...
            if (editResponse) {
                return (
                    <div>
                        ...
                        <SaveStyle disabled={disabledSave} sx={{marginRight: '0.5rem'}} onClick={saveResponse}>
                            Save
                        </SaveStyle>
                        <CancelStyle onClick={cancelResponse}>
                            Cancel
                        </CancelStyle>
                    </div>
                )
