            ...
            components={{
                p: ({  children, ...props }) => {
                    ...
                    const hasBlockElement = React.Children.toArray(children).some(child =>
                        React.isValidElement(child) && typeof child.type === "string" && ["div", "h1", "h2", "h3", "ul", "ol", "table"].includes(child.type)
                    );

                    // If block-level elements are found, avoid wrapping in <p>
                    return hasBlockElement ? <>{children}</> : <p {...props} style={{ whiteSpace: "pre-wrap" }}>{children}</p>;
                },
                a: ({  children, ...props }) => {
                    return (
                        //@ts-ignore
                ...
                code({ inline, className, children, }) {
                    ...
                    return (
                        <Suspense fallback={<code>Loading Code Block...</code>}>
                            {/*@ts-ignore*/}
                            <CodeRender cleanCode={children} inline={inline} language={(lang && lang[1]) || ""} />
                        </Suspense>
                    )
                }
            }}
