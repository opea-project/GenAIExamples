...
const DownloadChat = () => {
    ...
    const {selectedConversationHistory, type, model, token, temperature} = useAppSelector(conversationSelector)
    ...
    useEffect(()=>{
        ...
        if(selectedConversationHistory && selectedConversationHistory.length > 0 && selectedConversationHistory[userPromptIndex]){
    ...
    return url && type === 'chat' && (
        <Link download={`${fileName}.json`} to={url} target="_blank" rel="noopener noreferrer">
