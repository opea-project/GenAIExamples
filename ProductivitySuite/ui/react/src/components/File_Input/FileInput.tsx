...
import { UploadFile, Close, ExpandMore, FileUploadOutlined } from "@mui/icons-material";
import styles from "./FileInput.module.scss";
import { NotificationSeverity, notify } from "@components/Notification/Notification";
import { useAppDispatch, useAppSelector } from "@redux/store";
import { conversationSelector, setSourceFiles, setUploadInProgress, uploadFile } from "@redux/Conversation/ConversationSlice";
...
const dataExtensions = ["txt", "pdf",  "csv", "xls", "xlsx", "json", /*"doc", "docx", "md", "ppt", "pptx", "html", "xml", "xsl", "xslt", "rtf", "v", "sv"*/];
...
const FileInput: React.FC<FileInputProps> = ({ maxFileCount = 5, imageInput, dataManagement}) => {

    const { model, models, useCase, filesInDataSource, uploadInProgress, type } = useAppSelector(conversationSelector);
    ...
    const [filesToUpload, setFilesToUpload] = useState<(FileWithPreview|File)[]>([]);
    const [details, showDetails] = useState<boolean>(filesToUpload.length === 0)

    const inputRef = useRef<HTMLInputElement>(null);

    const extensions = imageInput ? imageExtensions : dataManagement ? dataExtensions : docExtensions;
    ...
    useEffect(()=>{
        ...
        if(!dataManagement){
            let selectedModel = models.find((thisModel)=>thisModel.model_name === model);
    ...
    const validateFiles = (newFiles: File[]) => {
        ...
        if (newFiles.length + filesToUpload.length > maxFileCount) {
            notify(`You can only upload a maximum of ${maxFileCount} file${maxFileCount > 1 ? 's' : ''}.`, NotificationSeverity.ERROR);
            return;
        }
        ...
        const validFiles = newFiles.filter((file) => {
            ...
            if (duplicate) {
                notify(`File "${file.name}" is already added.`, NotificationSeverity.ERROR);
                return false;
            }

            if (!isSupportedExtension) {
                notify(`File "${file.name}" has an unsupported file type.`, NotificationSeverity.ERROR);
                return false;
            }

            if (!isWithinSizeLimit) {
                notify(`File "${file.name}" exceeds the maximum size limit of ${imageInput ? '3MB' : '200MB'}.`, NotificationSeverity.ERROR);
                return false;
            }
    ...
    const removeFile = (index:number) => {
        let updatedFiles = filesToUpload.filter((file, fileIndex)=> index !== fileIndex);
                ...
                <Box className={styles.previewFiles}>
                    <Box className={styles.fileList}>
                        {filesToUpload.map((file, fileIndex)=>{
                            let activeFile = "file" in file ? file.file : file;
                            return <span key={fileIndex}><FileDispaly index={fileIndex} file={activeFile} remove={removeFile}/></span>
                        })}
                    </Box>
                </Box>
            )
            ...
            return (
                <ModalBox>
                    ...
                    <Box id="modal-modal-description" >
                        <p>I hereby certify that the content uploaded is free from any personally identifiable information or other private data that would violate applicable privacy laws and regulations.</p>
                        <div>
                            <SolidButton onClick={() => uploadFiles()}>Agree and Continue</SolidButton>
                            <OutlineButton onClick={() => setConfirmUpload(false)}>Cancel</OutlineButton>
                        </div>
                    </Box>
                </ModalBox>
            )
        ...
        <Box className={styles.fileInputWrapper}>
            ...
            <FileInputWrapper
                ...
                <div>
                    ...
                    {filesToUpload.length !== maxFileCount && (
                        <SolidButton onClick={()=>inputRef.current?.click()}>
                            ...
                            <input ref={inputRef} type="file" multiple accept={`.${extensions.join(',.')}`} /*multiple*/ hidden onChange={handleFileSelect} />
                        </SolidButton>
                    )}
                ...
                {filesToUpload.length > 0 && (
                    <ExpandButton className={`${styles.expand} ${details ? styles.open : ''}`}
                        onClick={()=>showDetails(!details)}>
                        <ExpandMore fontSize="small"/>
                    </ExpandButton>
                )}

               <div className={`${styles.details} ${details ? styles.detailsOpen : styles.detailGap}`}>
                    ...
                    <Typography
                        ...
                        color="textSecondary"
                        sx={{ mt: 0.5 }}>
                        You can select maximum of {maxFileCount} valid file{maxFileCount > 1 ? 's' : ''}.
                    </Typography>
                    ...
                    {!dataManagement && (
                        ...
                        color="textSecondary"
                        sx={{ mt: 0.5 }}>
                            Max supported input tokens for {imageInput && 'images'} data insight is {insightToken >= 1000 ? ((insightToken / 1000) + "K") : insightToken}
                        </Typography>
                    )}
