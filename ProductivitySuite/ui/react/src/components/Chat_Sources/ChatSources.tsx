...
const ChatSources: React.FC = () => {
    ...
    const { sourceLinks, sourceFiles, sourceType } = useAppSelector(conversationSelector);
    ...
    const renderElements = () => {
      return (
        sourceElements.map((element: any, elementIndex)=>{
          return <span key={elementIndex}><FileDispaly index={elementIndex} file={element.file} isWeb={isWeb}/></span>
        })
      )
    }
    ...
    return (
        <Box className={styles.sourceWrapper}>
            {renderElements()}
        </Box>
    );
};
