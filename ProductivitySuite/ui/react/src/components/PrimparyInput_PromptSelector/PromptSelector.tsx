    ...
    return prompts && prompts.length > 0 && (
        <Box ref={wrapperRef} className={styles.promptsWrapper}>
            ...
            <Tooltip title={!showPrompts ? "Show suggestions" : ""} arrow>
                <ExpandButton className={`${styles.expand} ${showPrompts ? styles.open : ''}`}
                    onClick={()=>setShowPrompts(!showPrompts)}>
                    <ExpandMore fontSize="small"/>
                </ExpandButton>
            </Tooltip>

            <PromptListWrapper className={`${styles.promptsListWrapper} ${showPrompts ? styles.open : ''}`} >
                        ...
                        return(
                            <ListItem key={promptIndex}>
                                <PromptButton 
                                    ...
                                    onClick={()=>handleSelect(prompt.prompt_text)}><span>{prompt.prompt_text}</span></PromptButton>

                                <Tooltip title="Remove suggestion" arrow>
                                    <IconButton 
                                        className={styles.delete} 
                                        onClick={() => handleDelete(prompt.id, prompt.prompt_text)}>
                                        <Delete />
                                    </IconButton>
                                </Tooltip>
