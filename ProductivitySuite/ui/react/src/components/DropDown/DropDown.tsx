...
import { List, ListItemButton, ListItemText, MenuItem, Menu, Typography, ListItemIcon, styled, Box } from '@mui/material';
...
const DropDown: React.FC<DropDownProps> = ({ options, value, handleChange, readOnly, border, ellipsis }) => {

	const [anchorEl, setAnchorEl] = useState<null | HTMLElement>(null);

	const foundIndex = options.findIndex(option => option.value === value);

	const [selectedIndex, setSelectedIndex] = useState(foundIndex !== -1 ? foundIndex : 0);
		...
		<Wrapper className={`${styles.dropDown} ${border ? styles.border : ''}`}>
			...
			<List>
				<ListItemButton onClick={handleClickListItem}>
					<ListItemText className={`${styles.noWrap} ${ellipsis ? styles.ellipsis : ''}`} primary={options[selectedIndex].name} />
					<ListItemIcon  className={styles.unsetMin}>
						<KeyboardArrowDownIcon fontSize='small' className={`${styles.chevron} ${open ? styles.open : ''}`}/>
					</ListItemIcon>
				</ListItemButton>
			</List>
