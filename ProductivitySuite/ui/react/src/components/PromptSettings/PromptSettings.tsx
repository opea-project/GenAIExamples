...
import { conversationSelector, setModel, setSourceType, setTemperature, setToken } from "@redux/Conversation/ConversationSlice";
...
const PromptSettings: React.FC<PromptSettingsProps> = ({readOnly = false}) => {

    const dispatch = useAppDispatch();
    
    const {models, type, sourceType, model, token, maxToken, temperature} = useAppSelector(conversationSelector)
    ...
    const [formattedModels, setFormattedModels] = useState<AvailableModel[]>(filterAvailableModels())
    ...
    const updateModel = (value: string) => {
        ...
        const selectedModel = models.find((model: Model) => model.model_name === value);
        if(selectedModel){
    ...
    const displaySummarySource = () => {
        ...
        if (sourceType === 'images' && type === 'summary') input = <FileInput imageInput={true} />;

        return (
            <div className={styles.summarySource}>{input}</div>
        )
    }
        ...
        return (
            <>
                ...
                <FormGroup sx={{marginRight: '1.5rem'}}>
                    <FormControlLabel sx={cursorDisable()} control={
                        <Box sx={{marginLeft: '0.5rem'}}>
                            <TokensInput value={token} handleChange={updateTokens} error={tokenError} readOnly={readOnly}/>
                        </Box>
                    } label={`Tokens${readOnly ? ': ' : ''}`} labelPlacement="start"/>
                </FormGroup>
                
                <FormGroup>
                    <FormControlLabel 
                    sx={cursorDisable()} 
                    control={<CustomSlider value={temperature} handleChange={updateTemperature} readOnly={readOnly}/>} 
            ...
            return (
                <>
                    ...
                    <FormGroup sx={{marginRight: '1.5rem', marginTop: '2rem'}}>
                        <FormControlLabel sx={cursorDisable()} control={<DropDown options={[
                        {name: 'Web', value: 'web'},
                        {name: 'Documents', value: 'documents'},
                        {name: 'Images', value: 'images'}
                    ]} border={true} value={sourceType} handleChange={updateSource} />} label="Summary Source" labelPlacement="start"/>
                    </FormGroup>
    ...
    return (
        <Box className={styles.promptSettingsWrapper}>
            ...
            <Box className={`${styles.promptSettings} ${readOnly ? styles.readOnly : ''}`}>
                    
                {formattedModels && formattedModels.length > 0 && (
                    <FormGroup key={formattedModels[0].name}>
                        <FormControlLabel 
                        sx={cursorDisable()} 
                        control={<DropDown options={formattedModels} value={model} handleChange={updateModel} readOnly={readOnly} border={true} ellipsis={true}/>} 
