...
import { SnackbarProvider, useSnackbar, MaterialDesignContent, closeSnackbar } from "notistack";
import { useEffect } from "react";
import { Subject } from "rxjs";
import {TaskAlt, WarningAmberOutlined, ErrorOutlineOutlined, InfoOutlined, Close} from '@mui/icons-material';
...
const StyledMaterialDesignContent = styled(MaterialDesignContent)<{ severity: AlertColor }>(({ variant }) => ({
    backgroundColor: (() => {
        switch (variant) {
            case "success":
        ...
        const subscription = Notify.subscribe({
            next: (notification) => {
                enqueueSnackbar(notification.message, {
                    ...
                    action: (key) => (
                        <CloseIcon onClick={()=>closeSnackbar(key)} variant={notification.variant}><Close/></CloseIcon>
                    )
                });
            }
        });
