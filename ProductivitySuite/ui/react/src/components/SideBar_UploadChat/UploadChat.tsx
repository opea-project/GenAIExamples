...
import { conversationSelector, getAllConversations, saveConversationtoDatabase, uploadChat } from "@redux/Conversation/ConversationSlice"
...
const UploadChat: React.FC<UploadChatProps> = ({ asideOpen, setAsideOpen }) => {
    ...
    const saveConversation = async () => {
        try {
            ...
            if (saveConversationtoDatabase.fulfilled.match(resultAction)) {
                ...
                notify("Conversation successfully uploaded", NotificationSeverity.SUCCESS);
        ...
        if (file) {
            ...
            reader.onload = () => {
                try {
                    ...
                    if(!fileContent.messages || !fileContent.model || !fileContent.token || !fileContent.temperature || fileContent.type){
                    ...
                    notify(`Error parsing JSON file: ${error}`, NotificationSeverity.ERROR);
    ...
    return (
        <>
            ...
            <LinkedMenuItem
                open={asideOpen}
                to="" sx={{ marginBottom: '2rem' }} onClick={handleUploadClick}>
