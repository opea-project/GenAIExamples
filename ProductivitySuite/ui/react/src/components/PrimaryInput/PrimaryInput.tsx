...
import { abortStream, conversationSelector, saveConversationtoDatabase, setSourceFiles, setSourceLinks } from '@redux/Conversation/ConversationSlice';
...
const PrimaryInput: React.FC<PrimaryInputProps> = ({ onSend, home = false }) => {

    const { onGoingResult, type, selectedConversationId, sourceLinks, sourceFiles } = useAppSelector(conversationSelector);
    ...
    const cancelStream = () => {
        ...
        if(type === 'chat'){
            dispatch(saveConversationtoDatabase({conversation: {id:selectedConversationId}}))
        }
    ...
    const submitButton = () => {
        if(!onGoingResult) {
            return (
                <CircleButton className={`${styles.circleButton} ${isActive() ? 'active' : '' }`} onClick={handleSubmit}><ArrowUpwardIcon /></CircleButton>
            )
        } return;
        ...
        if(!home && onGoingResult && (isSummary || isFaq)){
            return (
                <Box className={styles.primaryInput}>
                    <CircleButton className={styles.circleButton} onClick={cancelStream}><Stop /></CircleButton>
                </Box>
            )
            ...
            return (
                <Box className={styles.inputWrapper}>
                    ...
                    <InputWrapper className={`${styles.primaryInput} ${promptText ? 'active' : ''}`}>
                        ...
                        <TextInput
                            ...
                            value={promptText}
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => updatePromptText(e.target.value)}
                        ...
                        <Box className={styles.inputActions}>
                            ...
                            {onGoingResult && <CircleButton className={styles.circleButton} onClick={cancelStream}><Stop /></CircleButton>}
