...
function start_services() {
    ...
    n=0
    until [[ "$n" -ge 200 ]]; do
       docker logs audioqna-xeon-backend-server > $LOG_PATH/audioqna-xeon-backend-server.log 2>&1
       if grep -q complete $LOG_PATH/audioqna-xeon-backend-server.log; then
           break
       fi
       sleep 5s
       n=$((n+1))
