...
This deployment supports scalable operations for the following AI examples. You can specify the example name using the `--chart-name` flag:
- **chatqna**
- **codegen**
- **codetrans**
- **faqgen**
...
Note that only **chatqna** has `--with-rerank` support and can run with `--tuned` mode currently.
...
#### Case 1: Baseline Deployment for Chatqna with Rerank

Deploy Command (with node number, Hugging Face token, model directory specified):
```bash
python deploy.py --chart-name chatqna --hf-token $HF_TOKEN --model-dir $MODEL_DIR --num-nodes 2 --with-rerank
```
Uninstall Command:
```bash
python deploy.py --chart-name chatqna --uninstall
```

#### Case 2: Baseline Deployment for Chatqna without Rerank

```bash
python deploy.py --chart-name chatqna --hf-token $HFTOKEN --model-dir $MODELDIR --num-nodes 2
```

#### Case 3: Tuned Deployment for Chatqna with Rerank

```bash
python deploy.py --chart-name chatqna --hf-token $HFTOKEN --model-dir $MODELDIR --num-nodes 2 --with-rerank --tuned
```

#### Case 4: Tuned Deployment for codegen

```bash
python deploy.py --chart-name codegen --hf-token $HFTOKEN --model-dir $MODELDIR --num-nodes 2
```
...
```bash
bash benchmark.sh -n 2 -w chatqna
```
