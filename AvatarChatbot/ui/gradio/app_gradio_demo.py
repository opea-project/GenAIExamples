    ...
    with demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                with gr.Row():
                    gr.Markdown(f"""
                    <img src='data:image/png;base64,{gaudi_base64}' alt='Intel®Gaudi' style='width: 120px; height: auto;'>""", label="Intel®Gaudi")
                    gr.Markdown(f"""
                    <img src='data:image/png;base64,{xeon_base64}' alt='Intel®Xeon' style='width: 120px; height: auto;'>""", label="Intel®Xeon")
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                face_input = gr.File(file_count="single", file_types=["image", "video"], label="Choose an avatar or 📤 an image or video!")
        ...
        for i in range(len(image_pils)):
            image_click_buttons[i].click(update_selected_image_state,
                                         ...
                                         outputs=[face_input])
        for i in range(len(video_paths)):
            video_click_buttons[i].click(update_selected_image_state,
        ...
        submit_button.click(
            ...
            outputs=[audio_output_interm, 
                     interm_state,
                     audio_time_text,
                     chat_history_box], # need to change interm_state
        )
        interm_state.change(
            final_process,
