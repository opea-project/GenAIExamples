...
function start_services() {
    ...
    until [[ "$n" -ge 500 ]]; do
        ...
        docker logs tgi-gaudi-server > $LOG_PATH/tgi_gaudi_server_start.log
        docker logs asr-service > $LOG_PATH/asr_service_start.log

        if grep -q Connected $LOG_PATH/tgi_gaudi_server_start.log && grep -q "initialized" $LOG_PATH/asr_service_start.log; then
            break
        fi
       sleep 1m
       n=$((n+1))
